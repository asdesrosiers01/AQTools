...
**Update**: `station_crosswalk_sample.csv` now mirrors the *real* column
names and shape of the user's actual crosswalk (confirmed from an upload:
`FAA Identifier`, `WBAN ID`, `USAF ID`, `Data Period Starting/Ending`,
`Latitude`, `Longitude`, `GHCNh Filename`, `ISH Filename`, `Station Type
(ASOS/AWOS)`, `GHCN_ID`, `ICAO`, `Station Name`, `match_method`, `notes`
-- no `WMO` column and no `STATE` column at all). That first real run
surfaced two things the script now handles:
- `crosswalk_cols` needed updating to the real names (expected --
  that's exactly what the fail-loud column check is for).
- The crosswalk already carries its own lat/lon, period-of-record, and
  platform type -- better, more current data than a fresh MSHR join
  would give. The script now prefers the crosswalk's own values for
  those and only falls back to MSHR for what it truly lacks (elevation,
  UTC offset, state, WMO); `por_source` on each output row records which
  won. `ish_filename`/`ghcnh_filename`/`ghcn_id` are carried straight
  through for traceability, and the crosswalk's own `match_method`/`notes`
  feed a new `crosswalk_match_issue_flag` so a station the crosswalk
  already flagged as unverified doesn't win the "clean" pick over one
  that wasn't. Also: no station identifier role is hard-required
  anymore -- only "at least one of them produced a usable key
  somewhere" is, since a real crosswalk may legitimately lack any one
  of WMO/ICAO/FAA/USAF/WBAN.
